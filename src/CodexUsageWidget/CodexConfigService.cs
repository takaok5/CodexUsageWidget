...
internal static partial class CodexConfigService
{
    ...
    internal static string GlobalConfigPath => Path.Combine(CodexProjectReader.CodexHome, "config.toml");

    internal static string GetProjectConfigPath(string projectPath) =>
        Path.Combine(projectPath, ".codex", "config.toml");

    internal static ConfigWriteResult ApplyGlobal(ContextMode mode) => Apply(GlobalConfigPath, mode, false);

    internal static ConfigWriteResult ApplyProject(string projectPath, ContextMode mode)
    {
        if (string.IsNullOrWhiteSpace(projectPath) || !Directory.Exists(projectPath))
            return new ConfigWriteResult(false, projectPath, "The selected project is no longer available.");

        return Apply(GetProjectConfigPath(projectPath), mode, true);
    }
    ...
    private static ConfigWriteResult Apply(string configPath, ContextMode mode, bool isProject)
    {
        ...
        try
        {
            ...
            {
                string message = isProject
                    ? "No project context override is present; this project inherits the global setting."
            ...
            string successMessage = mode.IsDefault
                ? isProject
                    ? "Project context override removed. This project now inherits the global setting."
