...
internal static partial class CodexConfigService
{
    ...
    private static (string Original, string Updated) PrepareCatalogBootstrap(string catalogPath)
    {
        ...
        if (File.Exists(backupPath))
        {
            ...
            backupModels = FindGpt56Models(backupRoot);
        }

        bool changed = false;
        foreach ((string slug, JsonObject currentModel) in currentModels)
        {
            if (backupModels.TryGetValue(slug, out JsonObject? backupModel) &&
                backupModel["context_window"] is JsonNode originalContext &&
                !JsonNode.DeepEquals(currentModel["context_window"], originalContext))
            {
                currentModel["context_window"] = originalContext.DeepClone();
                changed = true;
            }
